...
import 'package:my_voicee/postLogin/reactions/donwVote/DownVoteListItem.dart';
import 'package:my_voicee/utils/Utility.dart';

class DownVoteScreen extends StatefulWidget {
  final data;
  final id;

  DownVoteScreen({this.data, this.id});

  @override
  _DownVoteScreenState createState() => _DownVoteScreenState();
}

class _DownVoteScreenState extends State<DownVoteScreen> {
  ...
  void initState() {
    ...
    _bloc.callApiGetReactions(context, widget.id, 3, page);
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.maxScrollExtent ==
        ...
        if (!isLoading) {
          isLoading = !isLoading;
          if (!lastPage) _bloc.callApiGetReactions(context, widget.id, 3, page);
        }
      }
    });
    super.initState();
      ...
      itemBuilder: (BuildContext context, int index) {
        return index == 0 && itemList[index].created_at == null
            ...
            : itemList[index].created_at == null
                ...
                : DownVoteListItem(fit: fit, pos: index, data: itemList[index]);
      },
