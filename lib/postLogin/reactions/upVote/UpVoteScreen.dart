...
import 'package:my_voicee/postLogin/reactions/upVote/UpVoteListItem.dart';
import 'package:my_voicee/utils/Utility.dart';

class UpVoteScreen extends StatefulWidget {
  final data;
  final id;

  UpVoteScreen({this.data, this.id});

  @override
  _UpVoteScreenState createState() => _UpVoteScreenState();
}

class _UpVoteScreenState extends State<UpVoteScreen> {
  ...
  void initState() {
    ...
    _bloc.callApiGetReactions(context, widget.id, 1, page);
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.maxScrollExtent ==
        ...
        if (!isLoading) {
          isLoading = !isLoading;
          if (!lastPage) _bloc.callApiGetReactions(context, widget.id, 1, page);
        }
      }
    });
    super.initState();
      ...
      itemBuilder: (BuildContext context, int index) {
        return index == 0 && itemList[index].created_at == null
            ...
            : itemList[index].created_at == null
                ...
                : UpVoteListItem(fit: fit, pos: index, data: itemList[index]);
      },
