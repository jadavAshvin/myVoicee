...
class OtherProfileScreen extends StatefulWidget {
  ...
  OtherProfileScreen({this.profileData});
...
class _OtherProfileScreenState extends State<OtherProfileScreen>
  ...
  void initState() {
    ...
    userDataBean = widget.profileData;
    ...
    _scrollController.addListener(() {
      if (_scrollController.position.maxScrollExtent ==
        ...
        if (!isLoading) {
          isLoading = !isLoading;
          if (!lastPage)
            _bloc.callApiGetAllPosts(context, widget.profileData.id, page);
        }
      }
    });
    page = 0;
      ...
      body: Stack(children: <Widget>[
        Container(
          margin: EdgeInsets.only(
              top: fit.t(8.0), left: fit.t(8.0), right: fit.t(4.0)),
          child: Stack(
            children: [
              Positioned(
                right: fit.t(0.0),
  ...
  void onClickReportUser() {
    showDialogReportUser(context,
        title: "Why are you reporting this account?",
        content: "", onReportClick: (item) {
      Navigator.of(context).pop();
      _bloc.callApiReportUser(context, item.title, widget.profileData.id);
    });
  }
}
