...
class ChannelListBloc implements ApiCallback {
  ...
  void onAPISuccess(Map<dynamic, dynamic> data, int flag) {
    ...
    if (flag == GET_ALL_POSTS) {
      ...
      var registerResponse = CommonApiReponse.fromJson(data);
      registerResponse.message = "Thank you for reporting the user. Our team will look at this account soon.";
      if (!apiResponse.isClosed) {
        apiResponse.add(registerResponse);
      }
      ...
      if (registerResponse.response != null) {
        writeStringDataLocally(key: userData, value: jsonEncode(registerResponse.response));
        if (!apiResponse.isClosed) {
          apiResponse.add(registerResponse);
        }
      }
